...
class DistrictGenerator:
    ...
    ----------
    prefix : string
        The two-letter state abbreviation (e.g., az, ut, ny).
    deviation : float
        The acceptable population deviation between districts (e.g., 0.05 for 5% population deviation).
    steps : int
        The number of steps to run the algorithm for, allowing it to further explore the possible maps.
    num_maps : int
        The number of maps to keep; keeps the top (num_maps) best maps based on the optimization metric.
    opt_metric_flag : str
    ...
    def _load_state_gdf(self) -> gpd.GeoDataFrame:
        """
        Loads the census block group data for the state we are generating districts for.
        ...
        print("Successfully loaded state GeoDataFrame")
        return geo_df

    def _get_random_partition(self) -> GeographicPartition:
        """
        Gets an initial random partition of the state to have a starting point for the algorithm.

        Returns
        -------
        GeographicPartition
            Resulting random partition of the state.
        """
    ...
    def _generate_maps(self, initial_partition: GeographicPartition) -> list[Tuple[float, list[GeographicPartition]]]:
        ...
        list[Tuple[float, list[GeographicPartition]]]
            List of tuples, where each tuple consists of the optimization metric for the map,
    ...
    def run(self) -> list[gpd.GeoDataFrame]:
        """
        The main runner function for the generator. Uses utilities in the class to generate district plans
