...
class DistrictGenerator:
    ...
    def run(self):
        ...
        for opt_metric, partition in best:
            ...
            self.state_gdf['District'] = self.state_gdf.index.map(partition)
            # Group by 'District' and sum the population
            district_population = self.state_gdf.groupby('District')['C_TOT22'].sum()
            ...
            self.state_gdf.plot(column='District', ax=ax, legend=True, cmap="rainbow")
